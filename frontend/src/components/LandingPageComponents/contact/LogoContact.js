import React, { useContext } from "react";
import { GlobalContext } from "../../../context/GlobalContext";
import "./LogoContact.css";

export const LogoContact = () => {
  const { UIstate } = useContext(GlobalContext);

  const handleModal = () => {
    UIstate.setisModalActive(true);
  };
  return (
    <div className="LogoContact">
      <div className="LogoContact__zIcon">
        <svg
          width="81"
          height="110"
          viewBox="0 0 81 110"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          onClick={handleModal}
        >
          <path
            d="M0 101.72L61.1486 12.3951V11.9033L4.84355 12.1341L4.79278 0.311131L79.7104 0V8.6615L18.9477 97.5447V98.0666L80.9492 97.8056V109.669L0 110V101.72Z"
            fill="url(#paint0_linear_16_5)"
          />
          <defs>
            <linearGradient
              id="paint0_linear_16_5"
              x1="7.14855"
              y1="-1.03376"
              x2="72.9412"
              y2="114.267"
              gradientUnits="userSpaceOnUse"
            >
              <stop stopColor="#006494" />
              <stop offset="0.2" stopColor="#026594" />
              <stop offset="0.27" stopColor="#096B94" />
              <stop offset="0.32" stopColor="#147493" />
              <stop offset="0.36" stopColor="#258192" />
              <stop offset="0.4" stopColor="#3B9291" />
              <stop offset="0.43" stopColor="#57A690" />
              <stop offset="0.46" stopColor="#77BF8E" />
              <stop offset="0.49" stopColor="#9BDB8C" />
              <stop offset="0.5" stopColor="#B4EE8B" />
              <stop offset="0.52" stopColor="#9FDC8E" />
              <stop offset="0.55" stopColor="#80C193" />
              <stop offset="0.58" stopColor="#64A997" />
              <stop offset="0.62" stopColor="#4E969A" />
              <stop offset="0.66" stopColor="#3D889D" />
              <stop offset="0.71" stopColor="#327D9F" />
              <stop offset="0.79" stopColor="#2B78A0" />
              <stop offset="1" stopColor="#2976A0" />
            </linearGradient>
          </defs>
        </svg>
      </div>

      <div className="LogoContact__digitizeIcon">
        <svg
          width="295"
          height="68"
          viewBox="0 0 295 68"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <g opacity="50%" clipPath="url(#clip0_16_6)">
            <path
              d="M0 18.2224C2.98557 17.7425 6.00457 17.5039 9.02803 17.5089C15.1064 17.5089 19.463 18.9607 22.3387 21.6596C25.2143 24.3585 26.9915 28.1618 26.9915 33.5038C26.9915 38.9202 25.3069 43.3005 22.2646 46.353C19.2224 49.4056 14.082 51.118 7.7198 51.118C5.14108 51.1349 2.56353 51.0024 0 50.7209L0 18.2224ZM4.26409 47.445C5.6791 47.6113 7.10395 47.6776 8.52819 47.6435C17.6241 47.6435 22.5238 42.5746 22.5238 33.6403C22.5732 25.8413 18.2042 20.9151 9.23784 20.9151C7.56976 20.8953 5.9038 21.0407 4.26409 21.3494V47.445Z"
              fill="white"
            />
            <path
              d="M50.8668 17.7693V50.8014H46.5965V17.7693H50.8668Z"
              fill="white"
            />
            <path
              d="M94.9579 49.3187C91.7194 50.5027 88.302 51.1156 84.8561 51.1303C79.87 51.1303 75.7663 49.8895 72.5698 46.7873C69.7312 44.0511 67.991 39.6398 67.9972 34.5026C67.9972 24.65 74.7852 17.4344 85.7632 17.4344C88.5747 17.3746 91.3691 17.8896 93.9767 18.9482L92.9215 22.4289C90.6373 21.4204 88.158 20.9371 85.6645 21.0143C77.6917 21.0143 72.4649 25.9778 72.4649 34.2792C72.4649 42.5807 77.4572 47.6063 85.1585 47.6063C87.9354 47.6063 89.8175 47.2154 90.7987 46.7128V36.8727H84.1341V33.4603H94.9579V49.3187Z"
              fill="white"
            />
            <path
              d="M117.334 17.7693V50.8014H113.069V17.7693H117.334Z"
              fill="white"
            />
            <path
              d="M147.589 21.374H137.611V17.7693H161.943V21.374H151.884V50.8014H147.62L147.589 21.374Z"
              fill="white"
            />
            <path
              d="M189.811 17.7693V50.8014H185.547V17.7693H189.811Z"
              fill="white"
            />
            <path
              d="M211.711 62.8814L248.879 7.66241V7.35839L214.661 7.5011V0.173723L260.153 0V5.35438L223.226 60.3004V60.623L260.9 60.4617L260.93 67.7953L211.73 68L211.711 62.8814Z"
              fill="url(#paint0_linear_16_6)"
            />
            <path
              d="M293.488 35.3153H280.776V47.2463H295.025V50.8014H276.512V17.7693H294.272V21.3182H280.776V31.785H293.488V35.3153Z"
              fill="white"
            />
          </g>
          <defs>
            <linearGradient
              id="paint0_linear_16_6"
              x1="216.056"
              y1="-0.639051"
              x2="257.078"
              y2="70.0352"
              gradientUnits="userSpaceOnUse"
            >
              <stop stopColor="#006494" />
              <stop offset="0.2" stopColor="#026594" />
              <stop offset="0.27" stopColor="#096B94" />
              <stop offset="0.32" stopColor="#147493" />
              <stop offset="0.36" stopColor="#258192" />
              <stop offset="0.4" stopColor="#3B9291" />
              <stop offset="0.43" stopColor="#57A690" />
              <stop offset="0.46" stopColor="#77BF8E" />
              <stop offset="0.49" stopColor="#9BDB8C" />
              <stop offset="0.5" stopColor="#B4EE8B" />
              <stop offset="0.52" stopColor="#9FDC8E" />
              <stop offset="0.55" stopColor="#80C193" />
              <stop offset="0.58" stopColor="#64A997" />
              <stop offset="0.62" stopColor="#4E969A" />
              <stop offset="0.66" stopColor="#3D889D" />
              <stop offset="0.71" stopColor="#327D9F" />
              <stop offset="0.79" stopColor="#2B78A0" />
              <stop offset="1" stopColor="#2976A0" />
            </linearGradient>
            <clipPath id="clip0_16_6">
              <rect width="295" height="68" fill="white" />
            </clipPath>
          </defs>
        </svg>
      </div>
    </div>
  );
};
